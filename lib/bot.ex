defmodule Finnbot.Bot do
  ...
  def handle_info({:joined, channel}, config) do
    ...
    Client.msg config.client, :privmsg, config.channel, "Joined!!"
    {:noreply, config}
  end
  ...
  def handle_info({:received, msg, %SenderInfo{:nick => nick}, channel}, config) do
    ...
    Commands.receive(%{msg: msg, nick: nick, channel: channel, config: config})
    {:noreply, config}
  end

  def handle_info({:mentioned, msg, %SenderInfo{:nick => nick}, channel}, config) do
    Logger.warn("#{nick} mentioned you in #{channel}: #{msg}")
    {:noreply, config}
  end
